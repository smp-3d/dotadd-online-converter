...
export default {
  ...
  methods: {
    doExport() {
      ...
      if (
        this.description != this.currentAdd.description ||
        this.description === ""
      )
        convo.options.push(
          new ConverterOption("description", this.description)
        );
      ...
      if (this.norm) convo.options.push(new ConverterOption("norm", this.norm));
      ...
      try {
        output = Converter.convert_string(
          [
            new ConvertableTextFile(
              this.currentFile.f.name,
              JSON.stringify(this.currentAdd)
            )
          ],
          convo
        );
      ...
      if (output.output_files.length)
        this.download(
          `${this.currentFile.f.name
            .split(".")
            .slice(0, -1)
            .join(".")}.${output.output_files[0].container}`,
          output.output_files[0].data
        );
    },
